...
def _load_model(system, checkpoint_path, device):
    ...
    if system == "superbpe":
        from systems.tokenization.superbpe.inference import load_checkpoint

        return load_checkpoint(checkpoint_path, device=device)
    if system == "manta":
        from systems.tokenization.manta.inference import load_checkpoint

        return load_checkpoint(checkpoint_path, device=device)
    if system == "fanta":
        from systems.tokenization.fanta.inference import load_checkpoint

        return load_checkpoint(checkpoint_path, device=device)
    if system == "flexitokens":
        from systems.tokenization.flexitokens.inference import load_checkpoint

        return load_checkpoint(checkpoint_path, device=device)
    if system == "magnet":
        from systems.tokenization.magnet.inference import load_checkpoint

        return load_checkpoint(checkpoint_path, device=device)
    if system == "fairtok":
        from systems.tokenization.fairtok.inference import load_checkpoint
...
def _build_induce_fn(system, model, device):
    ...
    raw, device))."""
    if system == "magnet":
        from systems.tokenization.magnet.segment import induce_spans
    ...
    if system == "fairtok":
        ...
        from systems.tokenization.fairtok.policy import segment_bytes
    ...
    if system == "flexitokens":
        from systems.tokenization.flexitokens.segment import induce_spans
    elif system == "manta":
        from systems.tokenization.manta.segment import induce_spans
    elif system == "fanta":
        from systems.tokenization.fanta.segment import induce_spans
    else:
...
def _build_induce_batch_fn(system, model, device):
    ...
    systems.tokenization.manta.segment.induce_spans_batch, padding the whole list to one
    ...
    if system in ("manta", "fanta"):
        from systems.tokenization.manta.segment import induce_spans_batch
