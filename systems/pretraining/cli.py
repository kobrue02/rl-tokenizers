"""Command-line entry point for systems.pretraining.train.train, mirroring every
systems/*/cli.py's shape: every TrainConfig field becomes a `--flag`,
generated from the dataclass itself.

Usage:
    python3 -m systems.pretraining.cli --shard-dir vocab_out/prep_run --model-size small --total-steps 20000
    torchrun --nproc_per_node=4 -m systems.pretraining.cli --shard-dir ... --model-size large ...

(data prep is a separate entry point -- see `python3 -m systems.pretraining.data_prep --help`,
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        description="Pretrain a TransformerLM over packed token shards (see systems.pretraining.data_prep)."
    )
