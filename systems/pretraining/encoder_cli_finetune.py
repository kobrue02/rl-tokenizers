...
the other three). Finetunes on one source language (English by convention,
matching Glot500's own run_tag.py/zero_shot_train.py protocol) and
evaluates zero-shot on a different target language.
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Finetune an encoder_train.py checkpoint on NER/POS/Taxi1500.")
    ...
    parser.add_argument("--task", choices=["ner", "pos", "taxi1500"], required=True)
    parser.add_argument("--train-lang", type=str, default="en", help="--task ner only -- WikiANN config, a 2-letter code")
    parser.add_argument("--eval-lang", type=str, default="de", help="--task ner only")
    ...
    parser.add_argument("--eval-config", type=str, default="de_gsd", help="--task pos only")
