...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        description="Evaluate a trained FlexiTokens checkpoint on held-out data."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        required=True,
        help="path to a FlexiTokens checkpoint (--output-dir at training time)",
    )
    parser.add_argument(
        "--eval-data-source",
...
def main(argv=None):
    ...
    report_eval(results, label="flexitokens")
    return results
