...
def build_arg_parser():
    ...
    for field in dataclasses.fields(MantaConfig):
        flag = "--" + field.name.replace("_", "-")
        help_text = f"(MantaConfig.{field.name}, default: {field.default})"
    ...
    parser.add_argument("--vocab-out", type=str, default="manta_vocab.json")
    parser.add_argument("--vocab-stats-out", type=str, default="manta_vocab_stats.json")
    parser.add_argument("--vocab-preview", type=int, default=20)
    return parser


def _config_from_args(args):
    field_names = {f.name for f in dataclasses.fields(MantaConfig)}
    kwargs = {k: v for k, v in vars(args).items() if k in field_names}
    return MantaConfig(**kwargs)
...
def main(argv=None):
    ...
    trainer = MantaTrainer(cfg, train_groups)
