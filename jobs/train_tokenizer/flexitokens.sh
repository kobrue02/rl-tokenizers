...
# comparison baseline for jobs/train_tokenizer/train.sh's fairtok policy.
# Usage: sbatch jobs/train_tokenizer/flexitokens.sh --data-source all --langs all --max-steps 20000 --vocab-size 50000
...
if [ $? -eq 0 ]; then
    ...
    sbatch jobs/eval/evaluate.sh flexitokens --checkpoint "$CHECKPOINT_PATH" --eval-data-source bouquet_test \
