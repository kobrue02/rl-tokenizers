...
# Usage: sbatch jobs/train_tokenizer/train.sh --data-source all --langs all --num-train-epochs 3 --lambda-target 20.0 --vocab-size 50000
...
if [ $? -eq 0 ]; then
    ...
    sbatch jobs/eval/evaluate.sh fairtok --checkpoint "$CHECKPOINT_PATH" --eval-data-source bouquet_test
else
