...
# MANTa baseline -- plain next-byte CE through a Gaussian-relaxed soft
# byte-to-block assignment, no reward/fairness machinery at all (unlike
# fairtok, the RL policy this is a comparison baseline for).
# Usage: sbatch jobs/train_tokenizer/manta.sh --data-source all --langs all --max-steps 20000 --vocab-size 50000
...
CHECKPOINT_PATH="$PROJECT_ROOT/checkpoints/manta_${SLURM_JOB_ID}.pt"
...
if [ $? -eq 0 ]; then
    ...
    sbatch jobs/eval/evaluate.sh manta --checkpoint "$CHECKPOINT_PATH" --eval-data-source bouquet_test \
        --output "results/manta_comparison.json" --result-key manta
else
