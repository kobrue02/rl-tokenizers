...
# (configs/eval_hf_frontier.yml), and BLT's entropy-based patcher
# (configs/eval_blt.yml). Submits one jobs/evaluate*.sh job per system --
...
# `sbatch jobs/combine_and_generate_figures.sh` directly if you want to force
...
#   # extra args forward to every jobs/evaluate.sh call for this project's OWN
...
#   # flags from configs/eval_hf_frontier.yml / configs/eval_blt.yml instead,
...
for tok in "${!PATTERNS[@]}"; do
    ...
    submit_if_stale "$tok" "results/${tok}_comparison.json" "$latest" \
        jobs/evaluate.sh "$tok" --checkpoint "$latest" \
        --eval-data-source bouquet_test --output "results/${tok}_comparison.json" "$@"
done

echo "=== External hf_frontier panel ==="
submit_if_stale "hf_frontier" "results/hf_frontier_comparison.json" "configs/eval_hf_frontier.yml" \
    jobs/evaluate_hf_frontier.sh -c configs/eval_hf_frontier.yml

echo "=== BLT (entropy-based dynamic patching) ==="
# ~16-20h job (see jobs/evaluate_blt.sh's own comment) -- the whole reason
# this script needs skip-if-up-to-date at all, not just for BLT's own sake.
submit_if_stale "blt" "results/blt_comparison.json" "configs/eval_blt.yml" \
    jobs/evaluate_blt.sh -c configs/eval_blt.yml

if [ "${#JOB_IDS[@]}" -eq 0 ]; then
    echo "Nothing to submit -- every system's results are already up to date."
    echo "(Run 'sbatch jobs/combine_and_generate_figures.sh' directly if you want to force-regenerate figures anyway.)"
    exit 0
fi
