...
# (no live fallback). Mirrors jobs/prep/glot500.sh's own AUTO-RESUBMIT
...
#
# Usage:
#   sbatch jobs/prep/pile.sh --output-dir "$WORK_ROOT/data/pile" --limit 4 --max-workers 4   # smoke test first
#   sbatch jobs/prep/pile.sh --output-dir "$WORK_ROOT/data/pile" --max-workers 8              # full ~451GB run
...
WORK_ROOT=/pfs/work9/workspace/scratch/tu_zxoqp65-rl-tokenizers  # same 44TiB scratch every jobs/*.sh uses -- see jobs/prep/pretraining_data.sh

# Preflight: fail fast if WORK_ROOT's own quota is maxed out (this ~451GB
# cache lands there, unlike jobs/prep/pretraining_data.sh's PROJECT_ROOT check)
# -- same check as jobs/prep/glot500.sh's own.
