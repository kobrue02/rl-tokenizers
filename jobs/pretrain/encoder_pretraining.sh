...
# jobs/pretrain/pretraining.sh's own structure for the decoder. UNLIKE the
...
# jobs/pretrain/pretraining.sh's own comment on this cluster's cpus-per-task
# <-> memory tie), since each rank's DataLoader spawns
# EncoderTrainConfig.num_workers=4 worker processes on top of its own main
# process.
#
# AUTO-RESUBMIT: identical mechanism to jobs/pretrain/pretraining.sh -- a run
...
#
# Usage:
#   sbatch jobs/pretrain/encoder_pretraining.sh --shard-dir pretrain_data/glot500_bpe \
#       --encoder-size base --total-steps 50000 --seq-len 512 --per-device-batch-size 16
#   sbatch --gres=gpu:4 jobs/pretrain/encoder_pretraining.sh --shard-dir pretrain_data/glot500_bpe \
...
# from it (see jobs/pretrain/pretraining.sh's identical reasoning).
CFG_INFO=$(python3 -c "
import sys
...
sbatch --gres=gpu:"$NUM_GPUS" --time="$TIME_LIMIT" --cpus-per-task="$CPUS_PER_TASK" jobs/pretrain/encoder_pretraining.sh "$@" --resume-from "$AFTER_CKPT"
SBATCH_EXIT=$?
if [ "$SBATCH_EXIT" -ne 0 ]; then
    echo "Resubmission via sbatch failed (exit $SBATCH_EXIT) -- resume manually with:" >&2
    echo "  sbatch --gres=gpu:$NUM_GPUS --time=$TIME_LIMIT --cpus-per-task=$CPUS_PER_TASK jobs/pretrain/encoder_pretraining.sh $@ --resume-from $AFTER_CKPT" >&2
    exit 1
fi
