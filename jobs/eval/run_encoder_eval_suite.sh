...
# jobs/combine/encoder_results.sh to run automatically once everything
# finishes. Separate jobs, not one chained job (mirrors
# jobs/eval/latest_checkpoints.sh's own reasoning): one tokenizer/
...
# jobs/prep/bible_nlp.sh) -- this script checks for data/bible_nlp and
...
# jobs/finetune/encoder.sh's own --time before finishing, and NO AUTO-RESUBMIT
...
#
# Usage:
#   bash jobs/eval/run_encoder_eval_suite.sh
...
# than jobs/finetune/encoder.sh's own 6h default even WITH the cap above (many extra
...
if [ ! -d "data/bible_nlp" ]; then
    ...
    echo "data/bible_nlp not found locally -- skipping roundtrip alignment for every" \
         "tokenizer (run 'sbatch jobs/prep/bible_nlp.sh' first if you want it included)."
fi
...
for label in "${!ENCODER_CHECKPOINTS[@]}"; do
    ...
    jobid=$(sbatch --parsable jobs/eval/encoder.sh "${COMMON_ARGS[@]}" \
    ...
    jobid=$(sbatch --parsable --time="$FINETUNE_SWEEP_TIME" jobs/finetune/encoder.sh "${COMMON_ARGS[@]}" \
    ...
    jobid=$(sbatch --parsable jobs/finetune/encoder.sh "${COMMON_ARGS[@]}" \
        --task taxi1500 \
...
COMBINEJOB=$(sbatch --parsable --dependency="afterany:$DEP" jobs/combine/encoder_results.sh)
