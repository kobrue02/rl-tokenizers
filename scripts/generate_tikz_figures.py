...
JSON file (e.g. results/hf_frontier_comparison.json, or a scripts/combine_eval_results.py output
...
Usage:
    ...
    python3 -m scripts.generate_tikz_figures -c configs/some_config.yml
...
# its entry is merged into the input file via scripts/combine_eval_results.py.
_PROVIDER_PANELS = [
...
def load_rows(results_path):
    ...
    if missing_spread:
        ...
        raise ValueError(
            ...
            f"run scripts/backfill_anchor_invariant_parity.py on it first: {missing_spread}"
        )
...
def build_arg_parser():
    ...
    parser.add_argument(
        ...
        "scripts/backfill_anchor_invariant_parity.py first if it doesn't)",
    )
    parser.add_argument(
